...
class LasrChess extends Program {
  ...
  acceptGame(computeInputs: IComputeInputs) {
    try {
      ...
      const { gameId, address1 } = txInputs
      ...
      return new Outputs(computeInputs, [updateGameStateInstruction]).toJson()
    } catch (e) {
      throw e
    }
  }
  ...
  makeMove(computeInputs: IComputeInputs) {
    try {
      const { from } = computeInputs.transaction
      const { move, fen, address1, gameId } = parseTxInputs(computeInputs)
  ...
  newGame(computeInputs: IComputeInputs) {
    try {
      ...
      return new Outputs(computeInputs, [updateProgramDataInstruction]).toJson()
    } catch (e) {
      throw e
    }
  }
